...
from app.models import Code, Country, ExternalRating, Genre, LogEntry, Show, TaskRun, ViewUser
...
def get_kp_mapping():
    ...
    mapping = {}
    for sid, url in qs.values_list('id', 'kinopoisk_url'):
...
def sync_poiskkino_ratings_task():
    ...
    all_country_names = {c['name'] for item in data_map.values() for c in item.get('countries', [])}
    ...
    if shows_to_update:
        ...
        Show.objects.bulk_update(shows_to_update, update_fields)
        ...
        Show.genres.through.objects.bulk_create(genre_relations, ignore_conflicts=True)
        ...
        Show.countries.through.objects.bulk_create(country_relations, ignore_conflicts=True)
