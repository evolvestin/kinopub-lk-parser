...
Object.assign(window.App, {
    ...
    doSearch: async function(q) {
        ...
        if (q.length < 2) {
            this.setState('data.search.results', null);
            resEl.innerHTML = `
                <div class="empty">
        ...
        this.searchTimer = setTimeout(async () => {
            try {
                ...
                this.setState('data.search.results', data);
            } catch (e) {
    ...
    renderSearchResults: function(data) {
        ...
        if (!data || (!data.shows?.length && !data.persons?.length)) {
            resEl.innerHTML = `<div class="empty"><div class="icon">${this.Icons.dash}</div>Ничего не найдено</div>`;
            return;
        }

        let html = '';
        if (data.persons?.length) {
            html += `<div class="label"><div class="icon" style="color:#d29922">${this.Icons.users}</div>Люди</div>`;
            ...
            data.persons.forEach(p => {
                ...
                const img = p.photo_url 
                    ...
                    : `<div class="person-avatar">${this.Icons.person_placeholder}</div>`;
                html += `
        ...
        if (data.shows?.length) {
            html += `<div class="label"><div class="icon" style="color:var(--info)">${this.Icons.film}</div>Контент</div>`;
            ...
            data.shows.forEach(s => {
                ...
                let badgesHtml = s.user_rating ? `<span class="rating-badge" style="background:rgba(0,0,0,0.6);border:none;">${this.Icons.star}${s.user_rating}</span>` : '';
                ...
                html += `
                    <div class="grid-item-wrap anim-item" onclick="window.App.openShowLayer(${s.id})">
                        <div class="grid-item">
                            ...
                            <button class="wishlist-add-btn" onclick="event.stopPropagation(); window.App.showFolderModal(${s.id}, '${safeTitle}')">${this.Icons.bookmark_plus}</button>
                        </div>
        ...
        requestAnimationFrame(() => this.fitAll('.grid-below-title', resEl));
    },
    ...
    handleImgErr: function(img, fallbackUrl, name) {
        if (fallbackUrl && !img.dataset.fallbackTried) {
            ...
            wrapper.innerHTML = this.Icons.person_placeholder;
